import React, {useState} from 'react';
import {Checkbox, Form, Input, message, Select} from 'antd';
import {useAction, useFetch} from '../../../helpers/hooks';
import {fetchSettings, postSettings} from '../../../helpers/backend_helper';
import {useEffect} from 'react';
const {Option} = Select;
import Button from '../../common/button';
import FormInput from "../../form/input";
import ImageInput from "../../form/image";
import {awsFileUpload} from "../../common/fileUploadAWS";


const FlutterwavePaymentMethod = () => {
    const [form] = Form.useForm()
    const [settings, getSettings] = useFetch(fetchSettings)
    const [image, setImage] = useState(null)

    useEffect(() => {
        if (settings?.flutterwave) {
            const data = settings?.flutterwave
            form.setFieldsValue({
                ...data
            })
        }
    }, [settings?.flutterwave])

    const onFinish = async (values) => {
        values.image = typeof values.image === 'object' && await awsFileUpload(values.image);
        const payload = {
            flutterwave: {
                ...values
            }
        }
        return useAction(postSettings, payload, () => {
            getSettings()
        })
    };

    return (
        <div className='border p-5'>
            <Form
                form={form}
                onFinish={onFinish}
                layout='vertical'
            >
                <FormInput
                    initialValue='flutterwave'
                    disableInput name={'name'}
                    label="Name"
                    placeholder="Name" required
                />

                <Form.Item name={'image'} label={"Image"}>
                    <ImageInput onSelect={(url) =>
                        setImage(url)
                    } />
                </Form.Item>

                <img className="h-28 pb-4" src={image || settings?.flutterwave?.image} alt="" />

                <Form.Item
                    label="Public key"
                    name={["credentials", "public_key"]}
                    rules={[
                        {
                            required: true,
                            message: 'Please input public api key!',
                        },
                    ]}
                >
                    <Input placeholder='Please Provide Public API key' />
                </Form.Item>
                <Form.Item
                    label="Secret key"
                    name={["credentials", "secret_key"]}
                    rules={[
                        {
                            required: true,
                            message: 'Please input secret api key!',
                        },
                    ]}
                >
                    <Input placeholder='Please Provide Secret API key' />
                </Form.Item>

                <Form.Item
                    label="Status"
                    name={["credentials", "active"]}
                    rules={[
                        {
                            required: true,
                            message: 'Please select payment status!',
                        },
                    ]}
                >
                    <Select
                        placeholder="Select Payment Status"
                    >
                        <Option value={true}>Enable</Option>
                        <Option value={false}>Disable</Option>
                    </Select>
                </Form.Item>
                <Button >Submit</Button>
            </Form>
        </div>
    );
};

export default FlutterwavePaymentMethod;